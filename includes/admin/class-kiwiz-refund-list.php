...
class Kiwiz_Refund_List extends WP_List_Table {
    ...
    function __construct(){
        ...
        parent::__construct( array(
            'singular'  => __('Refunds list', 'kiwiz-invoices-certification-pdf-file'),
            'plural'    => __('Refunds list', 'kiwiz-invoices-certification-pdf-file'),
            'ajax'      =>  false
        ) );
    }
    ...
    {
        $columns = array(
            'document_num'      => __('Refunds N°', 'kiwiz-invoices-certification-pdf-file'),
            'ID'                => __('Order Id', 'kiwiz-invoices-certification-pdf-file'),
            'document_date'     => __('Refunds date', 'kiwiz-invoices-certification-pdf-file'),
    ...
    public function column_default( $item, $column_name )
    {
        switch( $column_name ) {
            case 'ID':
                return '<a href="' .get_edit_post_link( $item['post_parent'] ). '" target="_blank">' .esc_html($item['post_parent']). '</a>';
                break;
            case 'document_date':
            ...
            case "document_status":
                if ( $item[ $column_name ] == 'certify')
                    return '<a class="kiwiz-list-grid dashicons-before dashicons-yes status-certify" onclick="return false;" href="" title="File Hash : ' . esc_attr(Kiwiz_Document_Certify::get_document_kiwiz_datas( $item['ID'], 'file_hash', 'refund' )) . "\n" . 'Block Hash : ' . esc_attr(Kiwiz_Document_Certify::get_document_kiwiz_datas( $item['ID'], 'block_hash', 'refund' )) . '"><strong>'.__('Certified','kiwiz-invoices-certification-pdf-file').'</strong></a>';
                        ...
                        if ( $item[ 'document_status' ] != 'nan' ) { ?>
                            <?php if ( $this->_is_kiwiz_activate && $item[ 'document_status' ] == 'certify' ) { ?>
                                <a class="button tips display_document"
                                   onclick="return manage_kiwiz_document('<?php echo esc_js(Kiwiz_Document_Certify::KIWIZ_GET_DOCUMENT_ACTION) ?>', 'refund', '<?php echo esc_js($item['ID']) ?>', '<?php echo esc_js(Kiwiz::get_wp_nonce()) ?>')"><?php echo __("Display", 'kiwiz-invoices-certification-pdf-file'); ?></a>
                            <?php } else if ( $item[ 'document_status' ] == 'no certify' ) { ?>
                                ...
                                <?php if (!Kiwiz_Document_Certify::is_certified_document($item['ID'], 'refund') && $this->_is_kiwiz_activate ) { ?>
                                    <a class="button tips create_certify_document"
                                       onclick="return manage_kiwiz_document('<?php echo esc_js(Kiwiz_Document_Certify::KIWIZ_CERTIFY_DOCUMENT_ACTION) ?>', 'refund', '<?php echo esc_js($item['ID']) ?>', '<?php echo esc_js(Kiwiz::get_wp_nonce()) ?>')"><?php echo __("Send to Kiwiz", 'kiwiz-invoices-certification-pdf-file'); ?></a>
                                    <?php
                                }
                            }
                        } else { ?>
                          <a class="button tips display_document" onclick="return manage_kiwiz_document('<?php echo esc_js(Kiwiz_Document_Certify::KIWIZ_GET_DOCUMENT_ACTION) ?>', 'refund', '<?php echo esc_js($item['ID']) ?>', '<?php echo esc_js(Kiwiz::get_wp_nonce()) ?>')"><?php echo __( "Display", 'kiwiz-invoices-certification-pdf-file' ); ?></a>
                        <?php }
                        ?>
                    </div>
                </div>
    ...
    /**
     * If the user has submitted the form, then validate the form data, update the settings, and return
     * a message. 
     * 
     * If the user has submitted the export form, then get the list of invoices, create a PDF of each
     * invoice, concatenate the PDFs, and redirect the user to the concatenated PDF. 
     * 
     * If the user has submitted neither form, then return nothing.
     * 
     * @return the array of the data that was posted.
     */
