...
class Kiwiz_Document_Invoice extends Kiwiz_Document {
    ...
    public function add_header() {
        echo '<table>
            <tr class="document-number">
                <td>' . __( "Invoice n°", 'kiwiz-invoices-certification-pdf-file' ) .' : '. esc_html($this->_document_number) .'</td>
                <td>' .__( "Order n°", 'kiwiz-invoices-certification-pdf-file' ) .' : '. esc_html($this->_order->get_id()) .'</td>
            </tr>
            <tr class="document-date">
                <td>' .__( "Invoice date", 'kiwiz-invoices-certification-pdf-file' ) .' : '. esc_html(date( parent::get_document_settings('shop_date_format'), $this->_document_date)) .'</td>
    ...
    public function add_order_items() {
        ...
        foreach ( $items as $item ) {
            ...
            echo '<tr class="tbody">
                ...
                <td class="ta-right">' .wp_kses_post($this->format_item_value(wc_price( wc_round_tax_total($item->get_subtotal() / $item->get_quantity()) ) )). '</td>
                <td class="ta-right">' .wp_kses_post($this->format_item_value( $item->get_quantity() )). '</td>                
                <td class="ta-right">' .wp_kses_post($this->format_item_value( wc_price( $item->get_subtotal_tax() ) )). '</td>
                <td class="ta-right">' .wp_kses_post($this->format_item_value( wc_price( wc_round_tax_total($item->get_subtotal() + $item->get_subtotal_tax()) ) )). '</td>
            </tr>';
    ...
    public function add_order_totals() {
        ...
        foreach( $this->_order->get_items('fee') as $item_id => $item_fee ) {
            echo '<tr class="fees">
                    <td class="column-product">' . __( "Fee", 'kiwiz-invoices-certification-pdf-file' ) .' :</td>
        ...
        if( $this->_order->get_discount_total() > 0 ) {
            echo '<tr class="discount">
                    <td class="column-product">' . __( "Discount", 'kiwiz-invoices-certification-pdf-file' ) .' :</td>
...
//        echo '<link rel="stylesheet" type="text/css" href="' . esc_url($style_file) . '">';
		wp_enqueue_style('kiwiz-invoice-dompdf-styles', $style_file);    }
